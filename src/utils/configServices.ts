...
const _responseConfig = async (response: Response) => {
  ...
  if (response.status === RESPONSE_STATUS.BAD_REQUEST) {
    throw Error('Mã vị trí không đúng! Vui lòng kiểm tra lại!');
  }
  throw Error('Máy chủ gặp lỗi. Vui lòng liên hệ quản trị viên');
};

const postService = async (url: string, body: object, header: any, isAuthorization = true, isFormData = false) => {
  try {
    if (url !== 'sys/login') {
    ...
    if (header) {
      Object.keys(header).map((key) => (headers[key] = header[key]));
    }
...
const getService = async (url: string, params?: any, body?: object | null, header?: any, isAuthorization = true) => {
  try {
    if (url !== 'sys/login') {
      // const lastTime = await AsyncStorage.getItem(ASYNC_STORE.LAST_TIME);
      // if (lastTime) {
